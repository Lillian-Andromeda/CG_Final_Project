...
SkyBox::SkyBox(const std::vector<std::string>& textureFilenames) {
    ...
    try {
        ...
        _myshader.reset(new GLSLProgram);
        _myshader->attachVertexShader(vsCode);
        _myshader->attachFragmentShader(fsCode);
        _myshader->link();
...
SkyBox::SkyBox(SkyBox&& rhs) noexcept
    : _vao(rhs._vao), 
      ...
      _myshader(std::move(rhs._myshader)) {
    rhs._vao = 0;
    rhs._vbo = 0;
}
...
void SkyBox::draw(const glm::mat4& projection, const glm::mat4& view) {
	...
	_myshader->use();
	_myshader->setMat4("projection", projection);
	_myshader->setMat4("view", glm::mat4(glm::mat3(view)));
